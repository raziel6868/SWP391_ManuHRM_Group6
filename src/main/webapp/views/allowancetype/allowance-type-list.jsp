...
<head>
    ...
    <title>Loại phụ cấp - ManuHRM</title>
        ...
        <div class="main-content">
            ...
            <div class="page-container">
                ...
                <div class="d-flex justify-content-between align-items-end mb-4 flex-wrap gap-3">
                    <div>
                        <h2 class="h3 text-on-surface fw-bold mb-1">Loại phụ cấp</h2>
                        <p class="body-md text-on-surface-variant mb-0">Quản lý danh mục phụ cấp dùng trong tính lương.</p>
                    </div>
                    <c:if test="${canCreate}">
                        <a href="${pageContext.request.contextPath}/allowance-type-create"
                ...
                <div class="card-premium overflow-hidden d-flex flex-column mb-4 w-100">
                    <div class="p-3 bg-surface border-bottom border-outline-variant">
                        <form action="${pageContext.request.contextPath}/allowance-type-list" method="GET"
                        ...
                        <table class="table table-premium mb-0 w-100">
                            ...
                            <tbody>
                                <c:forEach var="allowanceType" items="${allowanceTypes}">
                                    <tr <c:if test="${!allowanceType.isActive}">style="background-color: rgba(255,255,255,0.5); opacity: 0.8;"</c:if>>
                                        <td class="fw-medium text-on-surface">${allowanceType.code}</td>
                                        <td>${allowanceType.name}</td>
                                        <td>
                                            <span class="badge"
                                                  style="background-color: ${allowanceType.isTaxable ? '#dbeafe' : '#f3f4f6'}; color: ${allowanceType.isTaxable ? '#1e40af' : '#374151'};">
                                                ${allowanceType.isTaxable ? 'Có' : 'Không'}
                                            </span>
                                        </td>
                                        <td>
                                            <span class="badge"
                                        ...
                                        <td>
                                            <c:choose>
                                                <c:when test="${allowanceType.isActive}">
                                        ...
                                        <td class="text-end">
                                            <div class="d-flex justify-content-end gap-1">
                                                <c:if test="${canUpdate}">
                                                    <a href="${pageContext.request.contextPath}/allowance-type-update?id=${allowanceType.id}"
                                                       class="btn btn-sm btn-icon text-on-surface-variant hover-primary"
                                                       title="Sửa loại phụ cấp">
                                                        <span class="material-symbols-outlined" style="font-size: 1.25rem;">edit</span>
                                                    </a>
                                                </c:if>
                                                <c:if test="${canChangeStatus}">
                                                    <form action="${pageContext.request.contextPath}/allowance-type-status" method="POST" class="d-inline m-0">
                                                        <input type="hidden" name="id" value="${allowanceType.id}" />
                                                        <input type="hidden" name="isActive" value="${!allowanceType.isActive}" />
                                                        <button type="submit"
                                                                class="btn btn-sm btn-icon text-on-surface-variant hover-primary"
                                                                title="${allowanceType.isActive ? 'Vô hiệu hóa' : 'Kích hoạt'}"
                                                                onclick="return confirm('${allowanceType.isActive ? 'Bạn có chắc muốn vô hiệu hóa loại phụ cấp này?' : 'Bạn có chắc muốn kích hoạt lại loại phụ cấp này?'}');">
                                                            <span class="material-symbols-outlined" style="font-size: 1.25rem;">${allowanceType.isActive ? 'lock' : 'lock_open'}</span>
                                                        </button>
                                                    </form>
                                                </c:if>
                                            </div>
                        ...
                        <div class="p-3 bg-surface border-top border-outline-variant d-flex align-items-center justify-content-center">
                            <div class="d-flex gap-1 flex-wrap">
                                <c:forEach begin="1" end="${totalPages}" var="i">
                                    <a href="${pageContext.request.contextPath}/allowance-type-list?page=${i}&keyword=${keyword}&status=${selectedStatus}"
