...
<a href="user-form">+ Thêm nhân viên mới</a>
...
<c:if test="${currentPage > 1}">
    <a href="user-list?page=${currentPage - 1}&keyword=${keyword}&departmentId=${selectedDepartmentId}&roleId=${selectedRoleId}&isActive=${selectedStatus}&employeeType=${selectedEmployeeType}">« Trước</a>
</c:if>
<c:forEach begin="1" end="${totalPages}" var="i">
    <c:choose>
        <c:when test="${i == currentPage}">[${i}]</c:when>
        <c:otherwise>
            <a href="user-list?page=${i}&keyword=${keyword}&departmentId=${selectedDepartmentId}&roleId=${selectedRoleId}&isActive=${selectedStatus}&employeeType=${selectedEmployeeType}">${i}</a>
        </c:otherwise>
    </c:choose>
</c:forEach>
<c:if test="${currentPage < totalPages}">
    <a href="user-list?page=${currentPage + 1}&keyword=${keyword}&departmentId=${selectedDepartmentId}&roleId=${selectedRoleId}&isActive=${selectedStatus}&employeeType=${selectedEmployeeType}">Tiếp »</a>
</c:if>
